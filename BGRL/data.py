...
def download_fs_data(dataset):
    if dataset == 'Cora':
        return Planetoid(root='~/dataset/' + dataset, name="Cora")
    elif dataset == 'CiteSeer':
        return Planetoid(root='~/dataset/' + dataset, name="CiteSeer")
    elif dataset == 'CoraFull':
        return CoraFull(root='~/dataset/' + dataset)
    elif dataset == 'ogbn-arxiv':
        return PygNodePropPredDataset(root='./dataset/' + dataset, name="ogbn-arxiv")
    elif dataset == 'Coauthor-CS':
        return Coauthor(root='~/dataset/' + dataset, name="CS")
    elif dataset == 'Amazon-Computer':
        return Amazon(root='~/dataset/' + dataset, name="Computers")
    else:
        return None


def split(data, dataset):
    labels = torch.squeeze(data.y).tolist()
    class_list = [i for i in range(len(list(set(labels))))]
    ...
    return labels, train_class, dev_class, test_class, id_by_class
