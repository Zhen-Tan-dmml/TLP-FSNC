...
#for dataset in ["CoraFull","Coauthor-CS","Amazon-Computer","Cora","CiteSeer"]:
for dataset in ['ogbn-arxiv']:
#for dataset in ['ogbn-arxiv']:
    for avail_train_num_per_class in [0,10]:

        if dataset=='Amazon-Computer':args.epochs=2000
                ...
                for repeat in range(5):
                    ...
                    def pre_train(epoch, mode='train'):
                        ...
                        for j in range(fine_tune_steps):
                            ...
                            w, b = list(
                                map(lambda p: p[1] - fine_tune_lr * p[0], zip(grad, [w, b])))
                    ...
                    for epoch_test in range(args.test_epochs):
                        accs.append(pre_train(epoch_test, mode='test'))


                    results[dataset]['{}-way {}-shot {}-repeat'.format(N,K,repeat)]=[np.array(accs).mean(axis=0),
                                                                    np.std(np.array(accs))]

                    json.dump(results[dataset],open('./Meta-GNN-result_{}_{}_avail.json'.format(dataset,avail_train_num_per_class),'w'))
                    del model, adj
