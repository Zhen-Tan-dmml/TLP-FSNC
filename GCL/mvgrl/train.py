...
def train(dataset, verbose=False):
    ...
    hid_units = 512
    sparse = False
    n_way, k_shot, m_qry = 5, 5, 20
    test_num = 50

    adj, diff, features, labels, idx_train, train_class, dev_class, test_class, id_by_class = load(dataset)
    ...
    # for cla, idices in id_by_class.items():
    #     if cla not in train_class:
    #         continue
    #     feature = embeds[idices]
    ...
    for i in range(test_num):
        ...
        # if cali:
        #     beta = 0.5
        #     train_z = np.power(train_z[:, ], beta)
        #     test_z = np.power(test_z[:, ], beta)

        #     sampled_data = []
        #     sampled_label = []
        #     num_sampled = int(750 / k_shot)

        #     for i in range(n_way * k_shot):
        #         mean, cov = distribution_calibration(train_z[i], base_means, base_cov, k=2)
        #         sampled_data.append(np.random.multivariate_normal(mean=mean, cov=cov, size=num_sampled))
        #         sampled_label.extend([train_y[i]] * num_sampled)

        #     sampled_data = np.concatenate([sampled_data[:]]).reshape(n_way * k_shot * num_sampled, -1)

        #     z_aug = np.concatenate([train_z, sampled_data])
        #     y_aug = np.concatenate([train_y, sampled_label])

        #     clf = LogisticRegression(solver='lbfgs', max_iter=1000,
        #                              multi_class='auto').fit(z_aug, y_aug)
