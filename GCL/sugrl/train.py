...
def get_dataset(args, dataset_kwargs):
    if args.dataset_name in ['Cora', 'CiteSeer', 'PubMed', 'Photo', 'Computers', 'Coauthor', 'CoraFull']:
        if args.dataset_name in ['Cora', 'CiteSeer', 'PubMed']:
            ...
            dataset = Coauthor(root='~/dataset/' + args.dataset_name, name="CS")
        elif args.dataset_name == 'CoraFull'
            dataset =CoraFull(root='../dataset/' + args.dataset_name)
        data = dataset[0]
    ...
    return data, [A_I_nomal,adj_1], [data.x], [lable, nb_feature, nb_classes, nb_nodes], \
     id_by_class, train_class, dev_class, test_class
